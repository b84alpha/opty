...
if ! curl --max-time 20 -sS -D /tmp/stream_ok_headers.txt -N "$GATEWAY_URL/v1/chat/completions" \
  -H "Origin: $ORIGIN" \
...
acao=$(grep -i '^access-control-allow-origin:' /tmp/stream_ok_headers.txt | awk '{print $2}' | tr -d '\r')
if [[ -z "$acao" ]]; then
  echo "FAIL: missing Access-Control-Allow-Origin in response headers" >&2
  exit 1
fi
